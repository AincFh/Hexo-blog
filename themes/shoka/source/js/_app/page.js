const cardActive = function() {
  ...
  if (!window.IntersectionObserver) {
    ...
    // 修复 IntersectionObserver.observe 传参类型，防止报错，并增强健壮性和报错提示
    $.each('.index.wrap article.item, .index.wrap section.item', function(article) {
      if(article && article.nodeType === 1) {
        try {
          io.observe(article)
        } catch (err) {
          console.warn('IntersectionObserver.observe 失败，元素：', article, err)
        }
      } else {
        // 记录异常元素，便于后续排查
        console.warn('跳过非 Node 类型元素：', article)
      }
    })
